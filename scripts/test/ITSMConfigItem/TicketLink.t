...
################################
# test Ticket to CI item linkine
################################
...
for my $LinkType (@LinkTypes) {
    ...
    # create link to ConfigItem
    ok(
        $LinkObject->LinkAdd(
            SourceObject => 'Ticket',
            SourceKey    => $TicketID,
            TargetObject => 'ITSMConfigItem',
            TargetKey    => $TestLocation1ID,
            Type         => $LinkType,          #'RelevantTo',
            State        => 'Valid',
            UserID       => $UserID,
        ),
        'Ticket Linked to CI TestLocation1 ($LinkType)'
    );
    ...
    my $Source = exists $LinkList->{ITSMConfigItem}->{$LinkType}->{Source}
        ?
        $LinkList->{ITSMConfigItem}->{$LinkType}->{Source}->{$TestLocation1ID}
        :
        $LinkList->{ITSMConfigItem}->{$LinkType}->{Target}->{$TestLocation1ID};
    ok( $Source, 'Found Link' );

    is(
        $Source->{Class},
        'Country',
        'Ticket link has class Country'
    );

    is(
        $Source->{Name},
        'WellblechsHausen' . $LinkType . $RandomID,
        'Ticket link has correct Location Name'
    );
    ...
    ok(
        ...
        'TestLocation1 Name updated successfully.'
    );

    # check link data
    $LinkList = $LinkObject->LinkListWithData(
        Object => 'Ticket',
        Key    => $TicketID,
        UserID => $UserID,
        State  => 'Valid',
    ...
    $Source = exists $LinkList->{ITSMConfigItem}->{$LinkType}->{Source}
        ?
        $LinkList->{ITSMConfigItem}->{$LinkType}->{Source}->{$TestLocation1ID}
        :
        $LinkList->{ITSMConfigItem}->{$LinkType}->{Target}->{$TestLocation1ID};
    ...
    is(
        $Source->{Class},
        'Country',
        'Ticket link still has class Country'
    );

    is(
        $Source->{Name},
        'Niederkaltenkirchen' . $LinkType . $RandomID,
        'Ticket link has correct new Location Name'
    );
