...
// Rother OSS / CMDB
        ...
        $('<input>').attr({
            type: 'text',
            class: 'FilterBox',
            id: 'FilterDynamicFieldTypeBox',
            name: 'FilterDynamicFieldTypeBox'
        }).appendTo('#FilterDynamicFieldTypes');
        Core.UI.Table.InitTableFilter($('#FilterDynamicFieldTypeBox'), $('#DynamicFieldsTable'), 4);

        $( "#FilterDynamicFieldTypes" ).change(function() {
            let value = $(this).val();
            $( "#FilterDynamicFieldTypeBox" ).val(value).keydown();
        });
