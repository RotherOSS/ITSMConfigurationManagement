...
sub Data {
    ...
    $Self->{Translation}->{'TreeView for ConfigItem'} = 'Baumansicht für Configurations Items';
    $Self->{Translation}->{'Tree View'} = 'Baumansicht';
    $Self->{Translation}->{'Depth Level'} = 'Tiefe';
    $Self->{Translation}->{'Zoom In/Out'} = 'Vergrössern/Verkleinern';
    $Self->{Translation}->{'Max links level reached for ConfigItem!'} = 'Maximale Verschachtelungstiefe for ConfitItem erreicht!';
    $Self->{Translation}->{'DependsOn'} =  'Hängt ab von';
    $Self->{Translation}->{'PartOf'} = 'Ist Teil von';
    $Self->{Translation}->{'RelevantTo'} = 'Relevant für';
    $Self->{Translation}->{'RequiredFor'} = 'Benötigt für';
    $Self->{Translation}->{'AlternativeTo'} = 'Alternative für';
    $Self->{Translation}->{'ConnectedTo'} = 'Verbunden mit';
    ...
    $Self->{Translation}->{'Enables/disables the functionality to check ConfigItems for unique names. Before enabling this option you should check your system for already existing config items with duplicate names. You can do this with the script bin/otobo.ITSMConfigItemListDuplicates.pl.'} =
        '(De-)Aktiviert die Funktionalität um ConfigItems auf eindeutige Namen zu überprüfen. Bevor Sie diese Option aktivieren, sollten Sie Ihr System auf bereits vorhandene ConfigItems mit gleichem Namen überprüfen. Sie können dies mit Hilfe des scripts bin/otobo.ITSMConfigItemListDuplicates.pl tun.';
