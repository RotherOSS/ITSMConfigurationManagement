...
sub Data {
    ...
    $Self->{Translation}->{'Customer Company'} = 'Клиентская компания';
    ...
    $Self->{Translation}->{'Defines the shown columns of CIs in the config item widget depending on the CI class. Each entry must be prefixed with the class name and double colons (i.e. Computer::). There are a few CI-Attributes that are common to all CIs (example for the class Computer: Computer::Name, Computer::CurDeplState, Computer::CreateTime). To show individual CI-Attributes as defined in the CI-Definition, the following scheme must be used (example for the class Computer): Computer::HardDisk::1, Computer::HardDisk::1::Capacity::1, Computer::HardDisk::2, Computer::HardDisk::2::Capacity::1. If there is no entry for a CI class, then the default columns are shown as defined in the setting AgentCustomerInformationCenter::Backend###0060-CIC-ITSMConfigItemCustomerCompany (key DefaultColumns).'} =
        'Определяет отображаемые столбцы CI в виджете элемента конфигурации в зависимости от класса CI. Каждая запись должна иметь префикс с именем класса и двойное двоеточие (например, Computer::). Есть несколько CI-атрибутов, общих для всех CI (пример для класса Computer: Computer::Name, Computer::CurDeplState, Computer::CreateTime). Чтобы показать отдельные CI-атрибуты, определенные в CI-определении, необходимо использовать следующую схему (пример для класса Computer): Computer::HardDisk::1, Computer::HardDisk::1::Capacity::1, Computer::HardDisk::2, Computer::HardDisk::2::Capacity::1. Если для класса CI нет записи, то отображаются колонки по умолчанию, определенные в настройке AgentCustomerInformationCenter::Backend###0060-CIC-ITSMConfigItemCustomerCompany (ключ DefaultColumns).';
    ...
    $Self->{Translation}->{'Enables/disables the functionality to check ConfigItems for unique names. Before enabling this option you should check your system for already existing config items with duplicate names. You can do this with the script bin/otobo.ITSMConfigItemListDuplicates.pl.'} =
        'Включает/выключает проверку уникальности имен КЕ. До включения этого параметра, проверьте вашу БД на наличие дубликатов имен КЕ. Вы можете сделать это с помощью скрипта bin/otobo.ITSMConfigItemListDuplicates.pl.';
